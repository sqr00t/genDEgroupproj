...
def insert_csv_redshift(table: str, filename: str):    
        ...
        with closing(conn.cursor()) as cursor:
            ...
            if table == "orders_products":
                statements = [f"CREATE TEMP TABLE staging (LIKE deployment.orders_products);",
                             (f"COPY staging FROM 's3://team3-ordersproducts/{filename}'"
                ...
                statements = ["CREATE TEMP TABLE staging (LIKE deployment.stores);",
                            (f"COPY staging FROM 's3://team3-stores/{filename}'"
                              "IAM_ROLE 'arn:aws:iam::696036660875:role/RedshiftS3Role'"
                              "FORMAT AS CSV DELIMITER ',' "
                              "REGION AS 'eu-west-1';"),
                              """
                ...
                statements = ["CREATE TEMP TABLE staging (LIKE deployment.products);",
                            (f"COPY staging FROM 's3://team3-products/{filename}'"
                              "IAM_ROLE 'arn:aws:iam::696036660875:role/RedshiftS3Role'"
                              "FORMAT AS CSV DELIMITER ',' "
                              "REGION AS 'eu-west-1';"),
                              """
                ...
                statements = ["CREATE TEMP TABLE staging (LIKE deployment.orders);",
                            (f"COPY staging FROM 's3://team3-orders/{filename}'"
                              "IAM_ROLE 'arn:aws:iam::696036660875:role/RedshiftS3Role'"
                              "FORMAT AS CSV DELIMITER ',' "
                              "REGION AS 'eu-west-1';"),
                              """
                              ...
                              """,
                              "DROP TABLE staging"]
            else: logger.info(f"Invalid {table}")
